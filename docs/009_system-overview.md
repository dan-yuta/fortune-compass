...
**Fortune Compass** は、16 の占術（4カテゴリ: 定番・誕生日・伝統・特殊）で毎日の運勢を占える Web アプリケーションです。
...
| 誕生日 (Birthday) | 動物占い | 生年月日 | 動物キャラ（60パターン）・性格・相性 | いいえ（固定） |
...
| 画面 | パス | 説明 |
|------|------|------|
| トップ | `/` | ヒーローセクション + CTA ボタン + 16 占術アイコン（4カテゴリ） |
| プロフィール | `/profile` | 名前・フリガナ・生年月日・血液型の入力フォーム |
| 占術選択 | `/fortune` | ダッシュボードバナー + 16占術カード選択（4カテゴリ）。未登録なら `/profile` にリダイレクト |
...
│   └── fortune/
│       ├── page.tsx              #   占術選択（ダッシュボードバナー + 16占術カード）
...
└── lib/
    ...
    ├── api-client.ts             #   API 呼び出し（16 占術 + ダッシュボード）
    ├── useFortune.ts             #   占い共通フック（履歴自動保存付き）
    ├── fortune-registry.ts       #   占術レジストリ（16占術メタデータ）
...
| `POST /api/fortune/shichuu` | `{ birthday: "1990-05-15" }` | 天干地支・五行・性格・運勢 |
...
**アルゴリズム:** 生年月日から60パターンの動物キャラを算出。各キャラの性格・相性・行動パターンを返却。
...
| `frontend/src/app/fortune/page.tsx` | ~117 | 占術選択画面（ダッシュボードバナー + 16占術カード） |
...
| `frontend/src/lib/api-client.ts` | ~75 | API クライアント（16 占術 + ダッシュボード） |
| `frontend/src/lib/fortune-registry.ts` | 78 | 占術レジストリ（16占術メタデータ） |
