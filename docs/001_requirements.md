...
### 1.2 概要
19種の占術（4カテゴリ: 定番・誕生日・伝統・特殊）を統合し、総合運勢ダッシュボードを提供するWebアプリケーション。
...
| 7 | 動物占い | 生年月日 | JDN（ユリウス日）→Excelシリアル値→ `(serial+8)%60+1` で個性心理學60キャラを判定。SUN/EARTH/MOONグループ分類 | いいえ |
...
| 17 | 相性占い | 2人の生年月日等 | 星座・数秘・血液型の3軸で相性スコアを算出 | いいえ |
| 18 | 運勢トレンド | 生年月日等 | 7日間の運勢推移をSVGチャートで可視化 | はい |
| 19 | AI総合鑑定 | 生年月日等 | Anthropic SDK で複数占術結果を統合した総合鑑定文を生成 | はい |
...
| 3 | 占術選択 | `/fortune` | 19種の占術から個別 or ダッシュボードを選択 |
| 4-22 | 個別占い結果 | `/fortune/:type` | 占術ごとの詳細結果表示（19画面） |
| 23 | 総合運勢ダッシュボード | `/fortune/dashboard` | 4占術一括実行 + SVGレーダーチャート |
| 24 | 占い履歴 | `/history` | 過去の占い結果一覧（最大50件） |
| 25 | ヘルスチェック | `/health` | フロントエンドヘルスチェック |
| 26 | 404エラー | `/*` | カスタム404ページ |
