...
│  │          │                                                │  │
│  │  Step 4½: EC2 の状態チェック                               │  │
│  │          │ stopped → デプロイスキップ（成功扱い）          │  │
│  │          │ running → 次へ                                 │  │
│  │          │                                                │  │
│  │  Step 5: EC2 に SSH 接続してデプロイ実行（running時のみ）  │  │
│  └───────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
    │
    │  SSH（リモート接続）※ EC2 が running の場合のみ
    │
    ▼
┌─────────────────────────────────────────────────────────────────┐
│                   EC2 サーバー（k3s）                            │
...
| `paths:` | **特定のファイルが変更された場合のみ**トリガー |
| `workflow_dispatch:` | GitHub の画面から手動で「実行」ボタンを押したとき |

> **パスフィルタ**: `docs/` や `infra/` だけの変更ではデプロイは実行されません。
> アプリのコード（`backend/`, `frontend/`）やワークフロー自体（`deploy.yml`）が
> 変更された場合のみ自動デプロイが走ります。
>
> **例え話**: 「玄関のチャイムが鳴ったら（push）ドアを開ける」「自分でドアノブを回しても（手動）ドアは開く」
> という2つの開け方があるようなものです。
> ただし、チラシ（ドキュメント変更）が届いただけではドアを開けません。荷物（コード変更）が届いたときだけ開けます。
...
```
  ...
  │     │                                                      │
  │     ▼                                                      │
  │  Step 4½: EC2 の状態チェック（stopped→スキップ/running→続行）│
  │     │                                                      │
  │     ▼                                                      │
  │  Step 5: EC2 に SSH 接続して k3s のデプロイを更新           │
  │          （EC2 が running の場合のみ実行）                   │
  └────────────────────────────────────────────────────────────┘
```
      ...
      - name: Check EC2 instance state
        ...
        run: |
          ...
          if [ "$STATE" != "running" ]; then
            echo "::warning::EC2 instance is $STATE — skipping deploy."
          fi
```

SSH 接続の前に、EC2 インスタンスの状態を AWS API で確認します。
EC2 が `stopped`（停止中）の場合、SSH 接続を試みずにデプロイをスキップします。

- イメージは ECR に既にプッシュ済みなので、次に EC2 を起動したときに利用可能
- ワークフローは**成功扱い**（エラーにはならない）
- GitHub Actions の Annotations に warning メッセージが表示される

> **例え話**: お弁当を届ける前に「お店は開いているかな？」と電話で確認します。
> 閉まっていたら無駄足を踏まずに済みます。お弁当は冷蔵庫に保管してあるので安心です。

### Step 5: EC2 に SSH 接続してデプロイ

```yaml
      - name: Deploy to k3s via SSH
        if: steps.ec2-check.outputs.state == 'running'
...
> **補足**: 現在のワークフローでは、デプロイ前に EC2 の状態を自動チェックしています。
> EC2 が停止中の場合は SSH 接続を試みずにスキップするため、通常このエラーは発生しません。
> このエラーが発生する場合は、EC2 が起動途中（`pending` 状態）か、ネットワーク設定の問題です。
