...
|---------|------------|--------|--------------|
| ① トリガー | `.github/workflows/deploy.yml` | 3〜9行目 | `on: push` で master push を検知（`backend/**`, `frontend/**`, `deploy.yml` の変更時のみ） |
...
| ④½ EC2状態確認 | `.github/workflows/deploy.yml` | 72〜82行目 | EC2 が停止中ならデプロイをスキップ（イメージは ECR に保存済み） |
| ⑤ SSH＆デプロイ | `.github/workflows/deploy.yml` | 84〜131行目 | EC2 が running の場合のみ SSH 接続してサーバー上でコマンド実行 |
...
.github/workflows/deploy.yml
  ...
  1〜19行:   ワークフロー名・トリガー・権限・環境変数
             ├── name: Deploy to k3s
             ├── on: push (master) + paths フィルタ / workflow_dispatch
             │       └── backend/**, frontend/**, deploy.yml のみトリガー
 ...
 34〜70行:   Job 2 前半 — ビルド
             ├── AWS OIDC 認証（40〜44行）
             ├── ECR ログイン（46〜48行）
             ├── backend Docker ビルド＆push（50〜59行）
             └── frontend Docker ビルド＆push（61〜70行）

 72〜82行:   EC2 状態チェック
             └── EC2 が stopped なら SSH デプロイをスキップ（warning 出力）

 84〜131行:  Job 2 後半 — SSH でデプロイ（EC2 running 時のみ）
             ├── ANTHROPIC_API_KEY Secret 作成（97〜101行）
             ├── ECR プルSecret 更新（103〜110行）
             ├── イメージ更新 kubectl set image（113〜118行）
             ├── API キー注入 kubectl set env（121〜123行）
             ├── ロールアウト待機（126〜127行）
             └── Pod 状態確認（130行）
```
...
on:
  push:
    ...
    paths:                # ← 以下のファイルが変更された場合のみ
      - 'backend/**'      #    バックエンドのコード
      - 'frontend/**'     #    フロントエンドのコード
      - '.github/workflows/deploy.yml'  # ワークフロー自体の変更
  workflow_dispatch:       # ← GitHub画面から手動実行も可能
```

> **パスフィルタ**: `docs/` や `infra/` のみの変更ではデプロイは実行されません。
> アプリのコード変更がある場合のみ自動デプロイが走ります。
...
## 7½. EC2 状態チェック — 「お店が開いているか確認する」

### 何が起きるか

SSH でサーバーに接続する前に、EC2 インスタンスが起動しているかを AWS API で確認します。
...
EC2 が停止している状態で SSH 接続すると、30秒間のタイムアウト待ちの後にエラーになります。
このチェックがあれば、EC2 停止中でもワークフローは**成功扱い**になり、
イメージは ECR に保存されているので、次に EC2 を起動したときに利用できます。

| EC2 の状態 | テスト | ビルド/ECR push | SSH デプロイ |
|-----------|--------|----------------|-------------|
| running | 実行 | 実行 | 実行 |
| stopped | 実行 | 実行 | **スキップ（成功扱い）** |

> **例え話**: お弁当を届けに行く前に「お店が開いてるかな？」と電話で確認するようなものです。
> 閉まっていたら無駄足を踏まずに済みます。お弁当は冷蔵庫（ECR）に保管してあるので、
> 明日お店が開いたら届けられます。
...
④ ECR に送信（GitHub Actions → AWS ECR）               [1:30〜2:00]
   ...
   │
④½ EC2 状態チェック（GitHub Actions → AWS API）         [2:00]
   │  EC2 が停止中 → デプロイスキップ（イメージは ECR に保存済み）✓
   │  EC2 が起動中 → 次のステップへ
   │
